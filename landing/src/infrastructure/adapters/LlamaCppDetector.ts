...
/**
 * Infrastructure Adapter: LlamaCppDetector
 * High-performance LLM inference for Mac M-series via llama.cpp with Metal
 *
 * llama.cpp features:
 * - Native Metal acceleration for Apple Silicon
 * - Optimized for M1/M2/M3/M4 chips
 * - 200-500 tokens/s on M4 (comparable to vLLM on NVIDIA)
 * - Low memory footprint with quantization
 * - OpenAI-compatible API via llama-server
 *
 * Performance on Mac M4 (24GB RAM):
 * - Llama 3.1 8B Q4: ~300-400 tokens/s, ~6GB RAM
 * - Llama 3.1 8B Q8: ~200-300 tokens/s, ~9GB RAM
 * - Llama 3.1 8B FP16: ~150-200 tokens/s, ~16GB RAM
 *
 * Setup:
 * 1. Install llama.cpp:
 ...
 *      -ngl 999 -c 2048 --port 8080 --host 0.0.0.0
 * 4. Configure .env:
 *    ENABLE_LLAMACPP=true
 *    LLAMACPP_BASE_URL=http://localhost:8080
 *    LLAMACPP_MODEL=Meta-Llama-3.1-8B-Instruct-Q4_K_M
 */
export class LlamaCppDetector implements IPatternDetector {
  ...
  constructor(
    ...
    model: string = 'Meta-Llama-3.1-8B-Instruct-Q4_K_M'
  ) {
  ...
  getName(): string {
    return `llama.cpp (${this.model.split('/').pop()})`;
  }
  ...
  async detectPatterns(sequence: CandlestickSequence): Promise<TradingSignal> {
    ...
    try {
      ...
      if (this.requestCount === 0) {
        console.error(`\n❌ llama.cpp Error: ${errorMsg}`);
        console.error('   Make sure llama.cpp server is running:');
        console.error('   llama-server -m ./models/MODEL.gguf -ngl 999 --port 8080\n');
      }
  ...
  /**
   * Build the prompt for llama.cpp
   */
...
JSON Response:`;
  ...
  /**
   * Parse llama.cpp's response into a TradingSignal
   */
  private parseResponse(text: string): TradingSignal {
    try {
      ...
      return new TradingSignal(
        ...
        new Date(),
        0.5,
        `llama.cpp Analysis (unparsed): ${text.substring(0, 100)}...`
      );
    }
  }
